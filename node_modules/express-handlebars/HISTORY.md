...
* Fixed bug where showTable content was only be returned if a layout template was
...
* Updated internal `_resolveLayoutPath()` method to take the full
  `options`/locals objects which the view is showTable with. This makes it easier
  to override. ([#14][])
...
* Added support for render-level helpers, via `options.helpers`, to the
  ...
  (render-level) before a template is showTable; this provides flexibility at
  all levels. ([#3][], [#11][])
